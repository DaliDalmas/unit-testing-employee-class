...
class Manager(Employee):
    ...
    def __repr__(self):
        return f'Manager("{self.first_name}", "{self.middle_name}", "{self.sur_name}", {self.salary}, "{self._date_of_birth}", "{self.role}", {self._salary_percentage_increment}, "{self.team}")'


def main():
    print("Example employee class")
    emp = Manager("Dalmas", "DaliCodes", "Otieno", 6000, "1992-03-23", "CTO", 50, "Technology")
    print(emp)
